...
class CreateOffersEndpoint extends OfferManagementEndpointBase {
	...
	final protected function execute_endpoint( array $params ): array {
		...
		foreach ( $create_offers_data as $create_offer_data ) {
			try {
				...
				$coupon->add_meta_data( self::IS_FACEBOOK_MANAGED_POST_META_KEY, 'yes', true );
				$coupon->save();
	...
	private function create_coupon_from_offer_data( array $create_offer_data ): ?WC_Coupon {
		...
		if ( $percent_off > 0 ) {
			...
			$amount        = $this->parse_currency_amount_string( $fixed_amount_off_input, $errors );
		}
	...
	public static function parse_currency_amount_string( string $amount_with_currency, array &$errors ): ?string {
		// In the form of "10.50 USD"
		$amount_string_parts = explode( ' ', $amount_with_currency );

		if ( count( $amount_string_parts ) !== 2 || ( ! is_numeric( $amount_string_parts[0] ) ) ) {
			$errors[] = self::get_error_response_data( self::ERROR_OFFER_CREATE_FAILURE, sprintf( 'Invalid amount string: %s', $amount_with_currency ) );
			return null;
		}
