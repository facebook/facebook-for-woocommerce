	...
	final protected function execute_endpoint( array $params ): array {
		...
		foreach ( $codes_to_delete as $code ) {
			try {
				...
				if ( 'yes' !== get_post_meta( $coupon_id, self::IS_FACEBOOK_MANAGED_POST_META_KEY, true ) ) {
					$this->add_error( self::get_error_response_data( self::ERROR_OFFER_NOT_MANAGED_BY_FACEBOOK, '', $code ) );
					continue;
				}
