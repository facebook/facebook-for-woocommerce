...
class E2EProductCreator {
     ...
     */
    public static function createSimpleProduct($name, $price = 19.99, $stock = 10, $sku = null) {
        try {
            ...
            if ($sku) {
                ...
                // Generate unique SKU
                $product->set_sku('E2E-SIMPLE-' . time() . '-' . rand(1000, 9999));
            }
     ...
     */
    public static function createVariableProduct($name, $price = 29.99, $attributes = null) {
        try {
            // Verify WooCommerce is available
            if (!function_exists('wc_get_product')) {
                throw new Exception('WooCommerce not active');
            ...
            // Set SKU
            $product->set_sku('E2E-VARIABLE-' . time() . '-' . rand(1000, 9999));
...
if (php_sapi_name() === 'cli') {
    try {
        ...
        if ($product_type === 'simple') {
            $result = E2EProductCreator::createSimpleProduct($name, $price, $stock, $sku);
        } elseif ($product_type === 'variable') {
            $result = E2EProductCreator::createVariableProduct($name, $price);
        } else {
            $result = [
                'success' => false,
