...
async function publishProduct(page) {
  try {
    ...
    if (await publishButton.isVisible({ timeout: TIMEOUTS.LONG })) {
      ...
      await page.waitForTimeout(TIMEOUTS.MEDIUM); // Wait for publish to complete
  ...
  try {
    ...
    if (isVisualTabVisible) {
      ...
      if (isTextTabVisible) {
        ...
        await contentTextarea.waitFor({ state: 'visible', timeout: TIMEOUTS.MEDIUM });
...
async function exactSearchSelect2Container(page, locator, searchValue) {
  ...
  await firstResult.waitFor({ state: 'visible', timeout: TIMEOUTS.LONG });
