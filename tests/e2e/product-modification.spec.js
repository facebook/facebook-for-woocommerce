...
const { TIMEOUTS } = require('./test-constants');
const {
  baseURL,
  loginToWordPress,
  safeScreenshot,
...
test.describe('Facebook for WooCommerce - Product Modification E2E Tests', () => {
  ...
  test('Edit simple product and verify Facebook sync', async ({ page }, testInfo) => {
    ...
    try {
      ...
      await regularPriceField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      await trackStockCheckBox.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      if (!(await trackStockCheckBox.isChecked())) {
        await trackStockCheckBox.check();
      }
      await stockField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      titleField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
  ...
  test('Quick Edit simple product price and verify Facebook sync', async ({ page }, testInfo) => {
    ...
    try {
      ...
      await productRow.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      await quickEditLink.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      await quickEditRow.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      await updateButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      await updateButton.click();
      console.log('✅ Clicked Update button');

      // Step 10: Wait for the inline editor to close
      console.log('⏳ Waiting for Quick Edit form to close...');
      await quickEditRow.waitFor({ state: 'hidden', timeout: TIMEOUTS.ELEMENT_LONG });
      ...
      if (await priceColumn.isVisible({ timeout: TIMEOUTS.ELEMENT_STANDARD })) {
  ...
  test('Edit variable product and verify Facebook Sync', async ({ page }, testInfo) => {
    ...
    try {
      ...
      await variationsTab.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_QUICK });
      ...
      await expandAllButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_QUICK });
      ...
      await bulkActionsSelect.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_QUICK });
      ...
      if (await saveVariationsButton.isVisible({ timeout: TIMEOUTS.ELEMENT_QUICK }) && await saveVariationsButton.isEnabled({ timeout: TIMEOUTS.ELEMENT_QUICK })) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        await priceField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_QUICK });
  ...
  test('Edit Facebook-specific options for simple product', async ({ page }, testInfo) => {
    ...
    try {
      ...
      if (await fbPriceField.isVisible({ timeout: TIMEOUTS.ELEMENT_STANDARD })) {
      ...
      await useCustomImageRadio.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      await customImageField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
  ...
  test('Edit Facebook-specific options for variable product', async ({ page }, testInfo) => {
    ...
    try {
      ...
      const variationsTab = page.locator('li.variations_tab a');
      await variationsTab.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      await variationsTab.click();
      console.log('✅ Opened Variations tab');

      // Expand all variations
      const expandAllButton = page.getByRole('link', { name: 'Expand' }).first();
      await expandAllButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      await expandAllButton.click();
      console.log('✅ Expanded all variations');
      ...
      for (let i = 0; i < variationCount; i++) {
        ...
        if (await customImageRadioBtn.isVisible({ timeout: TIMEOUTS.ELEMENT_STANDARD })) {
          ...
          const customImageUrl = 'https://www.facebook.com/images/fb_icon_325x325.png';
          if (await customImageField.isVisible({ timeout: TIMEOUTS.ELEMENT_STANDARD })) {
            await customImageField.fill(customImageUrl);
      ...
      const saveVariationsButton = page.locator('button.save-variation-changes');
      if (await saveVariationsButton.isVisible({ timeout: TIMEOUTS.ELEMENT_STANDARD }) && await saveVariationsButton.isEnabled({ timeout: TIMEOUTS.ELEMENT_STANDARD })) {
        await saveVariationsButton.click();
        await page.waitForLoadState('domcontentloaded');
      ...
      await fbBrandField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      await fbBrandField.fill('FBOptionsUpdateTestBrand');

      const fbSizeField = page.locator('#fb_size');
      await fbSizeField.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
  ...
  test('Mark product as Out of stock and verify Facebook sync', async ({ page }, testInfo) => {
    ...
    try {
      ...
      await inventoryTab.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
      ...
      const trackStockCheckBox = page.locator('#_manage_stock');
      if (await trackStockCheckBox.isVisible({ timeout: TIMEOUTS.ELEMENT_QUICK })) {
        if ((await trackStockCheckBox.isChecked())) {
          await trackStockCheckBox.uncheck();
        }
      }
      const stockStatusSelect = page.locator('input[name="_stock_status"][value="outofstock"]')
      await stockStatusSelect.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_STANDARD });
