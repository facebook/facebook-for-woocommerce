...
class DeleteOffersEndpointTest extends OfferManagementAPITestBase
{
	...
	public function test_delete_single_offer(): void {
		...
		foreach([ $code_to_delete, $code_to_keep ] as $code) {
			...
			$coupon->add_meta_data( OfferManagementEndpointBase::IS_FACEBOOK_MANAGED_POST_META_KEY, 'yes', true );
	...
	public function test_delete_non_facebook_offer(): void {
		...
		$expected_error_data = [
			[
				'error_type' => OfferManagementEndpointBase::ERROR_OFFER_NOT_MANAGED_BY_META,
				'offer_code' => $code,
				'error_message' => null,
			],
		];
	...
	private static function get_request_params(array $offer_codes): array {
		$exp = time() + 120;
		return [
