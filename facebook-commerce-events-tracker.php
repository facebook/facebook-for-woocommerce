...
if ( ! class_exists( 'WC_Facebookcommerce_EventsTracker' ) ) :
	...
	class WC_Facebookcommerce_EventsTracker {
		...
		public function inject_purchase_event( $order_id ) {
			...
			// Add a flag to this order to ensure Purchase event is not fired multiple times throughout different sessions
			$order_meta = $order->get_meta_data();
			
			// Return if this Purchase event has already been tracked
			if ( 'yes' === get_transient( $purchase_tracked_flag ) || ( $order_meta && $order_meta->meta_exists( 'Meta_Purchase_Tracked' ) ) ) {
				return;
			}
